...
using CollectionNameStudent.Models;

namespace CollectionNameStudent.Services;
...
public class FirestoreService
{
    ...
    private async Task SetupFireStore()
    {
        if (db == null)
        {
            var stream = await FileSystem.OpenAppPackageFileAsync("CollectionNameStudent/Resources/Raw/namestudent-14086-firebase-adminsdk-cplxn-e734417795.json");
            var reader = new StreamReader(stream);
            var contents = reader.ReadToEnd();
            db = new FirestoreDbBuilder
            {
                ProjectId = "namestudent-14086",
